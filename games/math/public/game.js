...
// ─── Image loading ────────────────────────────────────────────────────────────
const gatorImg     = new Image(); gatorImg.src     = '/math/assets/alligator.png';
const appleImg     = new Image(); appleImg.src     = '/math/assets/apples.png';
const greenAppleImg= new Image(); greenAppleImg.src= '/math/assets/apple_green.png';
const monkeyImg    = new Image(); monkeyImg.src    = '/math/assets/monkey.png';
const gorillaImg   = new Image(); gorillaImg.src   = '/math/assets/gorilla.png';
const orangutanImg = new Image(); orangutanImg.src = '/math/assets/orangutan.png';
const parrotImg    = new Image(); parrotImg.src    = '/math/assets/parrot.png';
const backgroundImg= new Image(); backgroundImg.src= '/math/assets/background.png';
const heroImg      = new Image(); heroImg.src      = '/math/assets/hero.png';
const heroineImg   = new Image(); heroineImg.src   = '/math/assets/heroine.png';
const birdImg      = new Image(); birdImg.src      = '/math/assets/bird.png';
...
async function doLogin() {
  ...
  try {
    const r = await fetch('/math/api/login', {
      method: 'POST',
...
async function doRegister() {
  ...
  try {
    const r = await fetch('/math/api/register', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({username, password}),
    });
...
async function checkExistingAuth() {
  if (!authToken) return;
  try {
    const r = await fetch('/math/api/me', { headers: { Authorization: `Bearer ${authToken}` } });
    if (r.ok) {
      const data = await r.json();
      myDbUsername = data.username;
      myName = data.username;
...
function connectSocket() {
  gamePhase = 'waiting';
  socket = io({ path: '/math/socket.io', auth: { token: authToken || '' } });
